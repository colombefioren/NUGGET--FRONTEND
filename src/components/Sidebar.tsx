        ...
        <button type="button" onClick={onToggle} className="flex min-w-0 flex-1 items-center gap-2.5 text-start" aria-pressed={checked} title={t("library.include")}>
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-0.5 flex items-center gap-1.5 font-mono text-2xs text-subtle">
              <span className={cn("rounded border border-ink px-1 font-bold uppercase text-onpastel", kindColor(doc.kind))}>{doc.kind}</span>
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2 border-t-2 border-ink px-4 py-3">
        <span className="flex min-w-0 flex-1 items-center gap-2 rounded-full border-2 border-ink bg-raised px-2.5 py-1" title={health?.model}>
          ...
          <span className="truncate font-mono text-2xs font-bold">{online === false ? t("status.offline") : health ? health.model.split("/").pop() : "…"}</span>
        </span>
        {footerExtra}
        <ThemeToggle />
      </div>
