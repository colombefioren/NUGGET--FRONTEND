  ...
  return (
    <span
      className={cn(
        ...
        checked ? "border-fg bg-fg text-bg" : "border-fg/30 bg-transparent",
      )}
    >
      <AnimatePresence>
        {checked && (
...
function DocRow({ doc, checked, onToggle, onOpen, onDelete }: { doc: LibraryDoc; checked: boolean; onToggle: () => void; onOpen: () => void; onDelete: () => void }) {
  ...
  return (
    ...
    >
      <div className={cn("flex items-center gap-2.5 rounded-lg px-2 py-2 transition-colors hover:bg-sunken", !checked && "opacity-55 hover:opacity-100")}>
        ...
        <div className={cn("flex shrink-0 items-center transition-opacity", confirming ? "opacity-100" : "opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100")}>
          {confirming ? (
            <>
              <button type="button" className="btn px-1.5 py-1 text-2xs text-danger hover:bg-danger/10" onClick={onDelete}>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between px-5 pb-4 pt-5">
        ...
        >
          <Plus className="h-3.5 w-3.5" /> <span>{t("app.newChat")}</span>
        </button>
      </div>

      <div className="mx-5 grid grid-cols-2 rounded-lg bg-sunken p-0.5 text-xs" role="tablist">
        {(["library", "history"] as const).map((key) => (
          <button
            ...
            type="button"
            onClick={() => onTab(key)}
            className={cn("relative rounded-md py-1.5 transition-colors", tab === key ? "text-fg" : "text-muted hover:text-fg")}
          >
            {tab === key && <motion.span layoutId="tab" className="absolute inset-0 rounded-md bg-raised shadow-sm" transition={{ type: "spring", stiffness: 500, damping: 38 }} />}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-4 pt-4">
        {tab === "library" ? (
          <>
            <div className="mx-2 grid grid-cols-[1fr_auto] gap-2">
              <button
                type="button"
                onClick={() => fileRef.current?.click()}
                className="group flex items-center gap-2 rounded-lg border border-dashed border-fg/25 px-3 py-2.5 text-start text-xs text-muted transition-colors hover:border-accent/70 hover:text-fg"
              >
                <Upload className="h-3.5 w-3.5 shrink-0 transition-transform group-hover:-translate-y-0.5" />
              ...
              <button type="button" onClick={onPaste} className="btn-ghost w-11 border border-line" title={t("library.paste")} aria-label={t("library.paste")}>
            ...
            {docs.length > 0 && (
              <div className="mx-2 mb-1 mt-5 flex items-center justify-between">
                <span className="eyebrow">
                  {t("library.scope")} ·{" "}
                  {selected.length === docs.length
                    ? t("library.scopeAll")
                    : t("library.scopeSome", { count: selected.length, total: docs.length })}
                </span>
                <button
                  type="button"
            ...
            <ul className="mt-1">
              <AnimatePresence initial={false}>
                {pending.map((p) => (
                  <motion.li key={p.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="px-2 py-2">
                    <div className="flex items-center gap-2.5">
                      <span className="h-4 w-4 shrink-0 animate-spin rounded-full border-[1.5px] border-line border-t-accent" />
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[0.8125rem]">{p.name}</span>
                        <span className="relative mt-1 block h-px overflow-hidden bg-line">
            ...
            {docs.length === 0 && pending.length === 0 && <p className="mx-2 mt-6 text-xs leading-relaxed text-subtle">{t("library.empty")}</p>}
            <p className="mx-2 mt-6 font-mono text-2xs leading-relaxed text-subtle">{t("library.formats")}</p>
          </>
        ) : (
          <>
            {conversations.threads.length === 0 ? (
              <p className="mx-2 mt-2 text-xs leading-relaxed text-subtle">{t("history.empty")}</p>
            ) : (
              <ul>
                <AnimatePresence initial={false}>
                  {conversations.threads.map((th) => {
                    ...
                    return (
                      <motion.li key={th.id} layout exit={{ opacity: 0, height: 0 }} className="group relative">
                        ...
                        >
                          {active && <motion.span layoutId="thread-marker" className="absolute inset-y-2 start-0 w-0.5 rounded-full bg-accent" />}
                          <span className="block truncate text-[0.8125rem] text-fg" dir="auto">{th.title || t("history.untitled")}</span>
