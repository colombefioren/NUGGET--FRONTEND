...
export function Composer({ value, onChange, onSubmit, onStop, onFiles, onScopeClick, busy, followUp, scopeLabel, blocked, inputRef }: Props) {
  ...
  return (
    <form
      ...
      className="relative rounded-2xl border border-line bg-raised shadow-lift transition-colors focus-within:border-fg/30"
    >
      <textarea
        ref={inputRef}
        value={value}
      ...
      <div className="flex items-center gap-1 px-2 pb-2">
        ...
        <button
          ...
          className={cn(
            "btn-ghost h-8 min-w-0 px-2 text-xs",
            blocked && "text-danger hover:text-danger",
          )}
        >
          <Layers className="h-3.5 w-3.5 shrink-0" />
          <span className="truncate">{blocked ?? scopeLabel}</span>
        </button>
        <span className="ms-auto hidden pe-2 font-mono text-2xs text-subtle lg:block">{t("composer.hint")}</span>
        {busy ? (
          <motion.button
            type="button"
            onClick={onStop}
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
            className="btn-solid h-9 w-9 shrink-0 rounded-xl max-lg:ms-auto"
          ...
          <button
            type="submit"
            disabled={!canSend}
            className="btn h-9 w-9 shrink-0 rounded-xl bg-accent text-white hover:bg-accent/90 max-lg:ms-auto"
